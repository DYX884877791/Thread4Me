...
public class BooleanLockTest2 {
    ...
    public static void main(String[] args) {
        //
        final BooleanLock2 booleanLock2 = new BooleanLock2();
        Stream.of("T1","T2","T3","T4","T5").forEach(name -> {
            new Thread(() -> {
                try {
                    ...
                    booleanLock2.unlock();// 工作完成之后，自动释放锁，只有这里释放之后，isLocked变为false，才真正视为锁被释放
                }
            },name).start();
        });
        ...
        try {
             ...
             */
            booleanLock2.unlock();// 此时添加量标记线程，main线程调用unlock方法无效
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
